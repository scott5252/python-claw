# Quick Start For `examplev5`

This guide was prepared by reviewing Specs `001` through `017`, [`docs/env_settings.md`](/Users/scottcornell/src/my-projects/python-claw/docs/env_settings.md), and [`example/examplev5.md`](/Users/scottcornell/src/my-projects/python-claw/example/examplev5.md). The current codebase is a gateway-first, database-backed assistant platform with durable sessions, queued worker execution, approval-gated remote execution, child-agent delegation, webchat delivery, and audited diagnostics.

For `examplev5`, the important takeaway is this: the demo is not driven by one setting. It depends on a coordinated `.env` profile that turns on provider-backed LLM execution, webchat ingress, durable approvals, delegation to specialist agents, HTTP node-runner remote execution, and the bootstrap templates that teach each child agent how to call `remote_exec`.
...
Everything else in `.env.demo` is already shaped for `examplev5`. Still, those other values are required, because they control runtime mode, agent profiles, child-agent permissions, node-runner access, and webchat behavior.
...
11. Keep `PYTHON_CLAW_WEBCHAT_INTERACTIVE_APPROVALS_ENABLED=true` so approval prompts are rendered for the webchat surface.
12. Keep the auth and diagnostics tokens populated. The runtime and the admin inspection steps both depend on them.
13. If you change host ports such as `PYTHON_CLAW_POSTGRES_PORT` or `PYTHON_CLAW_REDIS_PORT`, also update your Docker usage accordingly. These two values affect host exposure, not the in-container service names used by `PYTHON_CLAW_DATABASE_URL`.
14. After editing `.env`, run the example with a fresh database volume if you previously booted the stack in `rule_based` mode. The agent/model bootstrap is seeded from current settings.

## `.env` Settings Needed For `examplev5`
...
The table below focuses on the variables that materially affect `examplev5`.

| Variable | What it does | Impact on `examplev5` | Example |
| --- | --- | --- | --- |
...
`examplev5` works only when all of these slices line up:
...
To get `examplev5` running, use `.env.demo` as the base, replace the OpenAI API key, and keep the rest of the demo profile intact. The demo specifically relies on:
...
If those stay aligned, the example flow in [`example/examplev5.md`](/Users/scottcornell/src/my-projects/python-claw/example/examplev5.md) matches the current code structure and durable database behavior.
