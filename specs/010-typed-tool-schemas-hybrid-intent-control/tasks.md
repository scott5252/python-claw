...
3. Add high-risk `remote_exec` schema tests first proving the open-key invocation schema accepts only the allowed scalar JSON value types, where the allowed set is exactly `string`, `number`, `boolean`, or `null`, rejects reserved backend-owned keys, rejects nested or non-scalar values, and excludes execution-envelope metadata from canonical arguments.
4. Add high-risk canonicalization and approval-identity tests first for the shared validation and serialization path in `src/policies/service.py` or a new shared helper, proving equivalent validated inputs serialize identically, field order does not affect hashes, schema-invalid inputs never produce approval-identity hashes, numeric and boolean `remote_exec` scalar values retain their validated JSON types during canonicalization, and governed approval identity changes when `tool_schema_name` or `tool_schema_version` changes without changing `typed_action_id`.
...
7. Add high-risk governance identity tests first proving governed schema identity is sourced from the governed resource payload using the required top-level payload shape, mirrored fields do not become an independent source of truth, and replayed approvals continue to match only when schema identity and canonical validated arguments both match.
...
9. Update `src/graphs/state.py` with the smallest additive type changes needed for one canonical bound-tool exposure contract carried on `AssistantState`, plus one graph-owned validated-call helper type and any bounded tool-error helper types, while preserving `ToolRequest`, `ToolEvent`, and `ModelTurnResult` compatibility where possible.
